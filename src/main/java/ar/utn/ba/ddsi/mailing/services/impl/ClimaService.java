...
public class ClimaService implements IClimaService {
  ...
  private static final String UBICACION_FIJA = "Buenos Aires";
  ...
  public Mono<Void> actualizarClima() {
    return obtenerClimaDeAPI(UBICACION_FIJA)
        .flatMap(clima -> {
          climaRepository.save(clima);
          logger.info("Clima registrado e historizado para: {} - Temp: {}°C, Humedad: {}%",
              clima.getCiudad(), clima.getTemperaturaCelsius(), clima.getHumedad());
          return Mono.empty();
        })
        .onErrorResume(e -> {
          logger.error("Error al conectarse a WeatherAPI: {}", e.getMessage());
          return Mono.empty();
        })
        .then();
  }
